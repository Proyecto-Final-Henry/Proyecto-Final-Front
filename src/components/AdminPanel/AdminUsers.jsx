    ...
    return (
        <div className={admin.component}>
            <div className={admin.list}>
                Usuarios Gratuitos
                {users && users.map(u => (u.active === true && u.role === "Gratuito" && u.confirmado === true) ? (
                    (
                        <div className={admin.user}>
                            <div> 
                                <img src={u.userImg} alt="usuario" className={admin.userImg} style={{width: '50px' , height: '50px', margin: "15px"}}/>
            ...
            <div className={admin.list}>
                Usuarios Premium
                {users && users.map(u => (u.role === "Premium") ? (
                    (
                        <div className={admin.user}>
                            <div> 
                                <img src={u.userImg} alt="usuario" className={admin.userImg} style={{width: '50px' , height: '50px', margin: "15px"}}/>
            ...
            <div className={admin.list}>
                Administradores
                {users && users?.map(u => (u.role === "Admin") ? (
                    (
                        <div className={admin.user}>
                            <div> 
                                <img src={u?.userImg} alt="usuario" className={admin.userImg} style={{width: '50px' , height: '50px'}}/>
                                <div>
                                    {u?.name}
                                    <br />
                                    Creado: {u.createdDate}
                                    <div>
                                        <button style={{margin:"3px"}} className="btn btn-secondary" onClick={() => unhandleAdmin(u.id)}> Quitar </button>
                                        {u.active ? <button style={{margin:"3px"}} className="btn btn-warning" onClick={() => handleDeactivate(u.id)}> Desactivar </button> : <button style={{margin:"3px"}} className="btn btn-warning" onClick={() => handleReactivate(u.id)}> Reactivar </button>}
                                        <button style={{margin:"3px"}} className="btn btn-danger" onClick={() => handleBan(u.id)}> Banear </button>
                                    </div>
            ...
            <div className={admin.list}>
                Usuarios Inactivos
                {users && users.map(u => (u.active === false && u.confirmado === true) ? (
                    (
                        <div className={admin.user}>
                            <div> 
                                <img src={u.userImg} alt="usuario" className={admin.userImg} style={{width: '50px' , height: '50px', margin: "15px"}}/>
